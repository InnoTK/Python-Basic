...
y=window.window_height()/2-size/2   #175
dx=0.1      #每次移動0.1
dy=0.1
while True: #無窮迴圈
    bob1.clear()
    ...
    if y>window.window_height()/2-size/2:
        dy = 0
    elif y<-window.window_height()/2+size/2:
        x = -dx ; dy = 0
    elif x>window.window_width()/2-size/2:
        dx = 0 ; dy = -dy
    elif x<-window.window_width()/2+size/2: 
        dx = 0 
    y += dy ; x += dx

    # if x>window.window_width()/2-size/2:       #275
    #     dx = 0 ; dy = -dy
    # elif x<-window.window_width()/2+size/2:    #-275
    #     dx = 0 ; dy = -dy
    # x += dx ; y +=dy

    # if y>window.window_height()/2-size/2:       #175
    #     dy = 0 ; dx = -dx
